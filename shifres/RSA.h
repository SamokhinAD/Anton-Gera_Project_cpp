 ...
 *          Этот алгоритм делает это быстро, используя двоичное представление степени.
 *          Основа всех вычислений в RSA
 */
...
int main_RSA(string key, int e, int text, int mod) {
    ...
    cout << "1. Ключ вводится как пара простых чисел одинаковой размерности - через пробел;" << endl;
    cout << "2. Значение открытой экспоненты выбирается в диапазоне от 2 до значения функции Эйлера от произведения ключей n и должно быть взаимно простым с ним;" << endl;
    ...
    try {
        ...
        cout << "\nВведите открытую экспоненту - число от 1 до " + std::to_string(f_n) + ":" << endl;
        if(e == -1)
        {
