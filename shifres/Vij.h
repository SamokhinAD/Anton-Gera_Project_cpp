 ...
 * @param alphabet Набор символов, которые можно шифровать (обычно буквы алфавита)
 ...
 * @param mod Режим работы: 0 - выбор с клавиатуры, 1 - шифрование, 2 - расшифрование
 * @return Результат работы - зашифрованный или расшифрованный текст
 * @details Шифр работает по принципу: каждая буква исходного текста складывается 
 *          с соответствующей буквой ключа (для шифрования) или вычитается (для расшифрования)
 */
string main_Vij(string alphabet, string key_text, string text, int mod) {
    ...
    cout << "2. Ключ составляется из символов веденного алфавита, его длина не должна превышать размеров будущего открытого текста;" << endl;
        ...
        if (_kbhit() || mod != 0) {
            ...
            if (arrow == 77 || mod == 1) {
                ...
                for (int i = 0; i < len_open_text; ++i) {
                    shifr_array[i] = (alphabet.find(open_text[i]) + alphabet.find(gamma_text[i])) % m;
                }
                ...
                /** @brief Гамма для расшифрования - то же ключевое слово, растянутое на длину текста */
                string gamma_text;
                ...
                for (int i = 0; i < len_shifr_text; ++i) {
                    open_array[i] = (alphabet.find(shifr_text[i]) - alphabet.find(gamma_text[i]) + m) % m;
                }
